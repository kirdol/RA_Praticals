...
We ensure that we have the necessary data for the analysis by filtering the temperature data for Nyon and removing any rows with missing values. We removed `r sum(is.na(data_temp$TAVG_C))` rows.
...
annual_maxima_nyon <- data_nyon %>%
  ...
  group_by(Year) %>%
  summarize(MaxTemp = max(TAVG_C), .groups = 'drop')
```
...
threshold_nyon <- quantile(data_nyon$TAVG_C, 0.95)

# Number of exceedances over the threshold
num_exceedances <- sum(data_nyon$TAVG_C > threshold_nyon)
```
...
gpd_fit <- fevd(TAVG_C, data = data_nyon, threshold = threshold_nyon, type = "GP")
```
