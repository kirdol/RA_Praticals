...
```{r}
# For the GEV model (Block Maxima approach)
negLL_gev_geneva <- gev_fit_geneva$results$negllh
k_gev <- length(gev_fit_geneva$results$par)
n_gev <- length(annual_maxima_geneva$MaxTemp) # sample size is # of annual maxima

aic_gev_geneva <- 2 * negLL_gev_geneva + 2 * k_gev
bic_gev_geneva <- 2 * negLL_gev_geneva + log(n_gev) * k_gev

cat("GEV (Geneva) AIC:", aic_gev_geneva, "\n")
cat("GEV (Geneva) BIC:", bic_gev_geneva, "\n")

# For the GPD model (POT approach)
negLL_gpd_geneva <- gpd_fit_geneva$results$negllh
k_gpd <- length(gpd_fit_geneva$results$par)
n_gpd <- length(extreme_values_geneva) # number of exceedances used in the fit

aic_gpd_geneva <- 2 * negLL_gpd_geneva + 2 * k_gpd
bic_gpd_geneva <- 2 * negLL_gpd_geneva + log(n_gpd) * k_gpd

cat("GPD (Geneva) AIC:", aic_gpd_geneva, "\n")
cat("GPD (Geneva) BIC:", bic_gpd_geneva, "\n")
```
