...
- For Lugano: `r round(extremal_index_lugano[1],2)`, for Geneva `r round(extremal_index_geneve[1],2)`, and for Säntis `r round(extremal_index_saentis[1],2)`. The extremal index ranges between 0 and 1. A value close to 0 indicates that extreme events are strongly clustered. A value closer to 1 suggests that extreme events are more isolated and independent. In this case, Lugano shows the most clustering of extremes (lowest index), while Säntis has more independent extreme events (highest index).
**Number of Clusters (Second Number):**
For Lugano: `r round(extremal_index_lugano[2],0)`, for Geneva `r round(extremal_index_geneve[2],0)`, and for Säntis `r round(extremal_index_saentis[2],0)`. This indicates the number of distinct clusters of extreme events detected for each station. More clusters suggest a higher frequency of extremes occurring over the studied period. This indicates the number of distinct clusters of extreme events detected for each station. More clusters suggest a higher frequency of extremes occurring over the studied period.
